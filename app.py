...
# ===============================
# Page config (一定要放第一個 Streamlit 命令)
# ===============================
st.set_page_config(
...
region = st.sidebar.selectbox("📍 分析地區（示範）", ["全台"], key="region")
crop = st.sidebar.selectbox("🌾 作物類型", ["水稻", "玉米", "高麗菜", "番茄"], key="crop")
...
if data is None:
    ...
    if st.button("🔄 抓最新資料", use_container_width=True):
        try:
            # 從 secrets 或環境變數抓 key（crawler.py 已處理）
            path, _ = crawl_and_save()
            st.success(f"✅ 已抓取並儲存：{path}")
            st.rerun()
...
temp_df = pd.DataFrame(
    {"預報日": [f"Day {i}" for i in range(1, 8)], "平均溫度 (°C)": temps}
)
